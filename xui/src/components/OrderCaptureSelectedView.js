...
import OrderCreateCapture from "./ordercreate/OrderCreateCapture";
import OrderCreateCaptureReview from './ordercreate/OrderCreateCaptureReview';
...
export default class OrderEditSelectedView extends Component {
    ...
    componentWillMount() {
        document.title = 'XSPACE | Order Photography'
    }
    ...
    updateSelectedStandard( evt,id) {
        ...
        this.setState( {selectedContentStandard: {name:name, uniqueID:uniqueID}}, ()=>console.log('selectedContentStandard', this.state.selectedContentStandard));
        this.setState( { noAddContentStandard: (uniqueID === 'null') ? true: false}, ()=> console.log('noAddContentStandard', this.state.noAddContentStandard));
    }
    ...
    gotoCart = () => {
        ...
        switch (this.state.selectedEdit) {
            case 1:  selectedCapture = 'singlePhoto';
                break;
            case 2:  selectedCapture =  'fourPack';
                break;
            case 3:  selectedCapture =  'fivePack';
                break;
            case 4:  selectedCapture =  'standard360';
                break;
        }
        console.log("CASES", selectedCapture)
        ...
        const listofslugs = this.state.productsForCart.map( entry => [ entry.xspaceid, selectedCapture, true, 'capture', contStand]);
        const fileList = this.state.productsForCart.map( entry => [ entry.name, entry.name]);
        ...
        //     const assetIDList = temp.map( entry => [entry[1], selectedCapture, true, 'edit']);
        ...
        const payload = {
            'orderid': neworderID,
            'orderparts': listofslugs,
            'fileList': fileList
        };

        console.log("GOTOCART", payload, this.state.productsForCart)

        this.props.submitCaptureOrder(payload)
        this.props.cancel()
    ...
    gotoChargebee() {
        console.log('GOTOCHARGEBEE', this.props.editurl);
        this.props.cancel();
    ...
    render() {
        ...
        switch (step) {
            case STEP_1_CREATE_PRODUCT_LANDING:
                ...
                break;
            case 2:
                fragment = <OrderCreateCaptureReview
        ...
        if (modeOption === MODE_CREATE_PRODUCT) {
            if (step === STEP_2_CREATE_PRODUCT_INPUT) {
                nextButton = <StyledButton2 successColor={false} onClick={this.nextStep} disabled={this.state.selectedEdit === 0}>Review{chevron}</StyledButton2>
